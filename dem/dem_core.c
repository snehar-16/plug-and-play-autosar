...
/* 100ms Persistence = 10 ticks of the 10ms Task  */
#define DID_CRC_VITAL          0xF107 /* [cite: 26] */

/**
 * @brief Required by main.c to ensure RAM is clean on boot [cite: 21]
 */
void Dem_Init(void) {
...
void Dem_SetEventStatus(uint16_t did, uint8_t isFailed) {
    ...
    if (isFailed) {
        ...
        if (isFault) {
            EventStatus[index] |= DEM_UDS_WIR; /* Buzzer ON per SMOP_SWRS_31  */
...
void Dem_ReportError(uint16_t did, uint8_t rawSignal) {
    ...
    /* 1. Explicit CRC Fault Bypass: Log within <10ms per SMOP_SWRS_33  */
    if (did == DID_CRC_VITAL && rawSignal == 1) {
        Dem_SetEventStatus(did, 1);
        return;
    }

    /* 2. Counter-Based Debounce (50ms window)  */
    if (rawSignal) {
        if (CounterDebounceTimer[index] < DEM_DEBOUNCE_FAIL_LIMIT) {
            CounterDebounceTimer[index]++;
        }
        if (CounterDebounceTimer[index] >= DEM_DEBOUNCE_FAIL_LIMIT) { 
            /* 3. Time-Based Debounce (100ms Persistence)  */
...
/**
 * @brief Main Loop (Called every 10ms from FreeRTOS task) [cite: 15, 21]
 */
void Dem_MainFunction(void) {
    Platform_WdgTrigger(); /* SIL-4 Watchdog Feed [cite: 12] */

    for (int i = 0; i < TOTAL_DTC_SLOTS; i++) {
        /* Aging: Auto-clear after 40 operation cycles  */
        ...
        /* Healing: 3 consecutive PASSED cycles clears WIR  */
